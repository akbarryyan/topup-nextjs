...
export default function PerformanceChart() {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-3 sm:p-4 lg:p-6 shadow-lg">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-3 sm:gap-0">
        <div>
          <h3 className="text-gray-900 text-base sm:text-lg font-bold mb-1 flex items-center gap-2">
            <svg
        ...
        <div className="flex flex-wrap items-center gap-1 sm:gap-2">
          {["7d", "30d", "90d"].map((range) => (
            <button
              ...
              className={`px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm transition-all duration-200 font-medium ${
                timeRange === range
      ...
      <div className="relative">
        ...
        <div className="h-48 sm:h-56 lg:h-64 relative">
          ...
          {/* Chart bars */}
          <div className="absolute left-8 sm:left-12 top-2 right-0 bottom-8 flex items-end justify-between gap-1 sm:gap-2">
            {chartData[timeRange].map((data, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex-1 flex items-end justify-center gap-1 group max-w-16"
                >
                  {/* Revenue bar */}
                  <div className="relative flex-1 max-w-[16px] sm:max-w-[20px]">
                    <div
                      className="bg-blue-600 rounded-t-sm transition-all duration-300 group-hover:bg-blue-700 relative"
                      style={{ height: `${revenueHeight}%` }}
                    >
                      {/* Tooltip */}
                      <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-1 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-800 text-white text-xs px-2 py-1 rounded whitespace-nowrap z-10">
                  ...
                  <div className="relative flex-1 max-w-[16px] sm:max-w-[20px]">
                    <div
                      className="bg-indigo-400 rounded-t-sm transition-all duration-300 group-hover:bg-indigo-500 relative"
          ...
          {/* X-axis labels */}
          <div className="absolute left-8 sm:left-12 bottom-0 right-0 flex justify-between text-xs text-gray-400">
            {chartData[timeRange].map((data, index) => (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-200">
        <div className="text-center">
          <div className="text-blue-600 text-lg sm:text-xl lg:text-2xl font-bold">
            {chartData[timeRange].reduce((sum, d) => sum + d.revenue, 0)}M
          </div>
          <div className="text-gray-500 text-xs sm:text-sm">Total Revenue</div>
        </div>
        <div className="text-center">
          <div className="text-gray-900 text-lg sm:text-xl lg:text-2xl font-bold">
        ...
        <div className="text-center">
          ...
          <div className="text-gray-500 text-xs sm:text-sm">Avg Revenue</div>
        </div>
        <div className="text-center">
          <div className="text-gray-900 text-lg sm:text-xl lg:text-2xl font-bold">
            {Math.round(
